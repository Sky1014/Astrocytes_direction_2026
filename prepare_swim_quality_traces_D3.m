% Prepare swim traces for recording-quality visualization.
%
% This script imports swim-related signals, performs preprocessing, and plots
% bilateral swim recording-quality views.
...
full_pdf = fullfile(out_dir, [char(data_id), '_bilateral_swim_full_window.pdf']);
zoom_pdf = fullfile(out_dir, [char(data_id), '_bilateral_swim_cycle03_zoom.pdf']);
full_pdf = Name_File_with_Suffix(full_pdf);
zoom_pdf = Name_File_with_Suffix(zoom_pdf);

full_stim_bar_intervals = collect_stim_on_intervals( ...
    orient, stages, swim_sample_window(1), swim_sample_window(end), SamplingFreq);

plot_bilateral_swim(time_swim_window, swim_ch1_mirror_ready, swim_ch2_mirror_ready, ...
    green_swim, pink_swim, full_pdf, full_stim_bar_intervals, [], [], 25);

[cycle_start, cycle_end, cycle_trial_id] = find_nth_complete_orientation_cycle( ...
    stages, swim_sample_window(1), swim_sample_window(end), 3);
zoom_mask = swim_sample_window >= cycle_start & swim_sample_window <= cycle_end;
zoom_direction_events = collect_stage_events(stages, cycle_trial_id, SamplingFreq);
zoom_stim_bar_intervals = collect_stim_on_intervals( ...
    orient, stages, cycle_start, cycle_end, SamplingFreq);

plot_bilateral_swim(time_swim_window(zoom_mask), ...
    swim_ch1_mirror_ready(zoom_mask), swim_ch2_mirror_ready(zoom_mask), ...
    green_swim, pink_swim, zoom_pdf, zoom_stim_bar_intervals, ...
    zoom_direction_events, make_direction_colors(), 10);
...
function plot_bilateral_swim(time_sec, ch1_up, ch2_down, green_swim, pink_swim, ...
    out_pdf, stim_bar_intervals, direction_events, direction_colors, scale_time_s)
fig = figure('Color', 'w', ...
    'Units', 'pixels', ...
    'Position', [250 250 760 420], ...
    'Visible', 'off');
ax = axes(fig, 'Position', [0.12 0.19 0.83 0.73]);
hold(ax, 'on');
